...
export function ProfileForm({ initialProfile }: { initialProfile: ProfileData }) {
   /* ── profile state (seeded dari server component) ── */
   ...
   // Profile di-fetch server-side (page.tsx) -> tak ada loading spinner &
   // loading.tsx route boundary jadi reachable. Hanya identitas petugas (loket)
   // yang client-only karena berasal dari sesi PIN.
   useEffect(() => {
      if (initialProfile.role !== "loket") return;
      let active = true;
      (async () => {
         try {
            const pinSession = await getPinSession();
            if (!active) return;
            if (pinSession?.petugas_nama && pinSession?.petugas_terminal_id) {
               setPetugasIdentity({
            ...
            // sesi PIN mungkin belum ada -> abaikan
         }
      })();
      return () => {
         active = false;
