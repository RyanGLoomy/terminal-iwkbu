...
/**
 * Mengembalikan label + warna untuk badge tenggat temuan.
 * - null/kosong → null (tidak ada badge)
 * - closed → null
 * - overdue → { label: "Terlambat X hari", color: red }
 * - due today → { label: "Jatuh tempo hari ini", color: red }
 * - due tomorrow → { label: "Jatuh tempo besok", color: amber }
 * - due in 2-3 days → { label: "X hari lagi", color: amber }
 * - due >3 days → null (tidak mendesak)
 */
export function getDueDateBadge(
   ...
   if (diffDays < 0) {
      return {
         ...
         color: "bg-red-100 text-error border-red-200 dark:bg-red-950/50 dark:text-red-300 dark:border-red-800",
      };
   }
   if (diffDays === 0) {
      return {
